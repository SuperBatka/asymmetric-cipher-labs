...
/* ALMOST PRODUCTION CODE char* Rsa::encrypt(const char *buffer, const size_t len, size_t *result_size, bool last_block)
{
   *result_size = 0;
   size_t left_count = len % (moduleByteCount-1);
   size_t count = 0;
   ...
   char *temp_data = new (std::nothrow) char[moduleByteCount];
   memset(result,0,size);
   ...
   for(size_t i = 0; i < count; i++)
   {
       ...
       size_t g;
    //   if ( i != (count-1) || left_count ==(moduleByteCount-1) || !last_block)
    //   {
    ...
    //       mpz_import(data, moduleByteCount-1, 1, sizeof(char), 0, 0, temp_data);
     ...
     //  {
      //     memcpy(temp_data,buffer + (moduleByteCount-1) * i,left_count);
     //      mpz_import(data, left_count, 1, sizeof(char), 0, -1, temp_data);
      // }
       if ( last_block && i == (count-1))
       {
           memcpy(temp_data,buffer + (moduleByteCount-1) * i,left_count);
       ...
       mpz_powm(encrypted_data,data,openExp_m,nModule_m);
       //qDebug()<<mpz_get_str(NULL,10,encrypted_data);
       size_t encrypted_data_byte_count = (mpz_sizeinbase(encrypted_data, 2) + 7) / 8;
       memset(temp_data,0,moduleByteCount);
...
char* Rsa::decrypt(const char *buffer, const size_t len, size_t *result_size, bool last_block)
{
   *result_size = 0;
   size_t left_count = len % moduleByteCount;
   size_t count = 0;
   ...
   for(size_t i = 0; i < count; i++)
   {
       ...
       mpz_powm(data,encrypted_data,closeExp_m,nModule_m);
       //qDebug()<<mpz_get_str(NULL,10,data);
       size_t encrypted_data_byte_count = (mpz_sizeinbase(data, 2) + 7) / 8;
       memset(temp_data,0,moduleByteCount);

       //if (i != (count-1))
     // /* if(i != (count-1) || left_count == moduleByteCount || !last_block)
     //  {
      ...
      // } else
      // {
       //    mpz_export(temp_data, &g, 1, sizeof(char), -1, 0, data);
       //   // memcpy(result + (moduleByteCount - 1) * i,temp_data,g);
       //    memcpy(result + *result_size,temp_data,g);
      //     *result_size+=g;
      // }
...
/*char* Rsa::encrypt(const char *buffer, const size_t len, size_t *result_size)
{
    *result_size = 0;
    ...
    for(size_t i = 0; i < count; i++)
    {
        memset(temp_data,0,moduleByteCount);
        size_t g;
        if ( i != (count-1) || left_count ==(moduleByteCount-1) || left_count == 0)
        {
            memcpy(temp_data,buffer + (moduleByteCount-1) * i,moduleByteCount-1);
            mpz_import(data, moduleByteCount-1, 1, sizeof(char), -1, 0, temp_data);
            //qDebug()<<mpz_get_str(NULL,10,data);
        } else
        {
            memcpy(temp_data,buffer + (moduleByteCount-1) * i,left_count);
            mpz_import(data, left_count, 1, sizeof(char), -1, 0, temp_data);
        }

        mpz_powm(encrypted_data,data,openExp_m,nModule_m);
        memset(temp_data,0,moduleByteCount);

        mpz_export(temp_data, &g, 1, sizeof(char), -1, 0, encrypted_data);
        memcpy(result + moduleByteCount * i,temp_data,moduleByteCount);
        *result_size+=moduleByteCount;
...
char* Rsa::decrypt(const char *buffer, const size_t len, size_t *result_size)
{
    *result_size = 0;
    ...
    for(size_t i = 0; i < count; i++)
    {
        ...
        size_t g = 0;
        memcpy(temp_data,buffer + moduleByteCount * i,moduleByteCount);
        mpz_import(encrypted_data, moduleByteCount, 1, sizeof(char), -1, 0, temp_data);
;
        ...
        //if (i != (count-1))
        if(i != (count-1) || left_count == moduleByteCount || left_count == 0)
        {
            mpz_export(temp_data, &g, 1, sizeof(char), -1, 0, data);
            memcpy(result + (moduleByteCount - 1) * i,temp_data,moduleByteCount-1);
            *result_size+=(moduleByteCount - 1);
    ...
    *result_size = 0;
    ...
    for(size_t i = 0; i < count; i++)
    {
        ...
        if ( i != (count-1) || left_count ==(moduleByteCount-1) || left_count == 0)
        {
            ...
            mpz_import(data, moduleByteCount-1 - k, 1, sizeof(char), 0, 0, temp_data+k);
        } else
        {
            memcpy(temp_data,buffer + (moduleByteCount-1) * i,left_count);
            while(temp_data[k] == 0)
            ...
            mpz_import(data, left_count-k, 1, sizeof(char), 0, 0, temp_data+k);
        }
        ...
        memcpy(result + k + moduleByteCount * i,temp_data,g);
        *result_size+=g;
    }
...
char* Rsa::decrypt(const char *buffer, const size_t len, size_t *result_size)
{
    *result_size = 0;
    ...
    for(size_t i = 0; i < count; i++)
    {
        ...
        mpz_import(encrypted_data, moduleByteCount-k, 1, sizeof(char), 0, 0, temp_data+k);
        ...
        memset(temp_data,0,moduleByteCount);
            ...
            memcpy(result + k + *result_size,temp_data,g);
            *result_size+=g;
...
/* almost workable char* Rsa::encrypt(const char *buffer, const size_t len, size_t *result_size)
{
    *result_size = 0;
    size_t left_count = len % (moduleByteCount-1);
    size_t count = 0;
...
/*char* Rsa::encrypt(const char *buffer, const size_t len, size_t *result_size)
{
    *result_size = 0;
    ...
    for(size_t i = 0; i < count; i++)
    {
        ...
        {
            memcpy(temp_data,buffer + (moduleByteCount-1) * i,size-1-*result_size);
            mpz_import(data, size-1-*result_size, 1, sizeof(char), 0, 0, temp_data);
            //memcpy(temp_data,buffer + (moduleByteCount-1) * i,size-1-*result_size);
            //mpz_import(data, left_count, 1, sizeof(char), 0, 0, temp_data);
            //qDebug()<<mpz_get_str(NULL,10,data);
        }
