...
Rsa_MPW::Rsa_MPW(QWidget *parent) : QWidget(parent)
{
    ...
    save_file_button    = new QPushButton("Сохранить в");
    ...
    digits_line_edit->setValidator( new QIntValidator(2, 999, this) );
    ...
    openedFile_label    = new QLabel("Имя файла");
    gen_prime_label     = new QLabel("Количество знаков в простом числе: ");
    saveFile_label      = new QLabel("Результирующий файл:");
    ...
    file_layout->addWidget(openedFile_label,0,0,1,1);
    file_layout->addWidget(openedFile_line_edit,1,0,1,2);
    file_layout->addWidget(saveFile_label,2,0,1,1);
    file_layout->addWidget(saveFile_line_edit,3,0,1,2);
    file_layout->addWidget(open_file_button,4,0,1,1);
    file_layout->addWidget(save_file_button,4,1,1,1);
    file_layout->addWidget(encrypt_file_button,5,0,1,1);
    file_layout->addWidget(decrypt_file_button,5,1,1,1);
    ...
    //glayout->addWidget(back_to_menu_button);




    setLayout(glayout);
    //connect(back_to_menu_button, SIGNAL(clicked()),this, SLOT(menuButtonHandler()));
    ...
    connect(open_file_button, SIGNAL(clicked()),this, SLOT(openFileButtonHandler()));
    connect(save_file_button, SIGNAL(clicked()),this, SLOT(saveFileButtonHandler()));
...
void Rsa_MPW::menuButtonHandler()
{
    emit test_emit();
}
...
void Rsa_MPW::calculateNumbersButtonHandler()
{
    ...
    if(!rsaCore.setPrimeNumbers(pPrimeStr.toLocal8Bit().constData(), qPrimeStr.toLocal8Bit().constData())) {
        QMessageBox::warning(this,"Warning", "P и Q должны быть простыми и больше 17");
        return;
    }
    printParamToForm(true);
    QMessageBox::information(this,"Info", "Ключи были успешно вычислены и установлены");
    return;
}

void Rsa_MPW::openFileButtonHandler()
{
    inputFileName = QFileDialog::getOpenFileName(this,"Open the file");
    openedFile_line_edit->setText(inputFileName);
}
...
void Rsa_MPW::saveFileButtonHandler()
{
    resultFileName = QFileDialog::getSaveFileName(this, tr("Save File"));
    saveFile_line_edit->setText(resultFileName);
}
...
void Rsa_MPW::generatePrimeNumbersButtonHandler()
{
    ...
    if (v->validate(digits_number, pos) != QValidator::Acceptable){
            QMessageBox::warning(this,"Warning", "Выбирете размер простого числа в промежутке от 2 до 999");
            return;
        }
...
void Rsa_MPW::printParamToForm(bool full)
{
    size_t size = 0;
    char *module = nullptr;
...
void Rsa_MPW::rsaText(Operation OP)
{
    ...
    if(OP == Operation::ENCRYPT)
    {
        QByteArray textByteArray( text.toLatin1() );
        res = rsaCore.encrypt(textByteArray.constData(),textByteArray.size(),&size);
    ...
    if(OP == Operation::DECRYPT)
    {
        QByteArray textByteArray = QByteArray::fromBase64(text.toLocal8Bit());
        qDebug()<<textByteArray.size();
        res = rsaCore.decrypt(textByteArray.constData(),textByteArray.size(), &size);
        QString result  = QString::fromLocal8Bit(res,size);
        output_text_edit->clear();
...
void Rsa_MPW::rsaFile(Operation OP)
{
    ...
    QString opencheck = openedFile_line_edit->text();
    if (opencheck == "")
    {
        QMessageBox::information(this,"Info", "Выберете входной файл");
        openFileButtonHandler();
        if(inputFileName == "")
        {
            return;
        }
...
void Rsa_MPW::calculateRsaFile(Operation OP)
{
    ...
    quint64 blcSize = rsaCore.getModuleByteCount();
    int bufferSize = (4096 / blcSize) * blcSize;
   ...
   for (quint64 i = 0; i < bufferBlocksCount; i++)
   {
       ...
       if(OP == Operation::ENCRYPT)
       {
           result = rsaCore.encrypt(buffer,bytesRead, &res_size);
       }
